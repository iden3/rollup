...
describe("ONCHAINTX forceWithdraw", () => {
    ...
    it("onchaintx forcewithdraw error pass", (done) => {
        const out = process.exec("cd ..; node cli.js onchaintx --type forceWithdraw --amount 2 --tokenid 0");
        out.on("exit", (code) => {
            expect(code).to.not.be.equal(0);
            done();
        });
    ...
    it("onchaintx forcewithdraw error amount", (done) => {
        const out = process.exec(`cd ..; node cli.js onchaintx --type forceWithdraw --pass ${pass} --tokenid 0`);
        out.on("exit", (code) => {
            expect(code).to.not.be.equal(0);
            done();
        });
...
describe("OFFCHAINTX", () => {
    it("offchaintx send", (done) => {
        const out = process.exec(`cd ..; node cli.js offchaintx --type send --pass ${pass} --amount 2 --to 12 --tokenid 0 --fee 0.1 --paramsTx ./src/resources/config-example.json`);
        out.stdout.on("data", (data) => {
            expect("200\n").to.be.equal(data);
            done();
        });
    ...
    it("offchaintx send default config.json", (done) => {
        const out = process.exec(`cd ..; node cli.js offchaintx --type send --pass ${pass} --amount 2 --to 12 --tokenid 0 --fee 0.1`);
        out.stdout.on("data", (data) => {
            expect("200\n").to.be.equal(data);
            done();
        });
    ...
    it("offchaintx send error pass", (done) => {
        const out = process.exec("cd ..; node cli.js offchaintx --type send --amount 2 --to 12 --tokenid 0 --fee 0.1 --paramsTx ./src/resources/config-example.json");
        out.on("exit", (code) => {
            expect(code).to.not.be.equal(0);
            done();
        });
    ...
    it("offchaintx send error amount", (done) => {
        const out = process.exec(`cd ..; node cli.js offchaintx --type send --pass ${pass} --to 12 --tokenid 0 --fee 0.1 --paramsTx ./src/resources/config-example.json`);
        out.on("exit", (code) => {
            expect(code).to.not.be.equal(0);
            done();
        });
    ...
    it("offchaintx send error recipient", (done) => {
        const out = process.exec(`cd ..; node cli.js offchaintx --type send --amount 2 --pass ${pass} --tokenid 0 --fee 0.1 --paramsTx ./src/resources/config-example.json`);
        out.on("exit", (code) => {
            expect(code).to.not.be.equal(0);
            done();
        });
    ...
    it("offchaintx send error token id", (done) => {
        const out = process.exec(`cd ..; node cli.js offchaintx --type send --amount 2 --pass ${pass} --to 12 --fee 0.1 --paramsTx ./src/resources/config-example.json`);
        out.on("exit", (code) => {
            expect(code).to.not.be.equal(0);
            done();
        });
    ...
    it(" offchaintx send error config file", (done) => {
        const out = process.exec(`cd ..; node cli.js offchaintx --type send --pass ${pass} --amount 2 --to 12 --tokenid 0 --fee 0.1 --paramsTx ./src/resources/config-examplee.json`);
        out.on("exit", (code) => {
            expect(code).to.not.be.equal(0);
            done();
        });
