...
describe('CREATE KEYS IMPORT', async function () {
    this.timeout(10000);
    it('create wallet test', (done) => {
        process.exec(`cd ..; node cli.js createkeys --keytype rollup --mnemonic "obscure property tackle faculty fresh gas clerk order silver answer belt brother" --pass ${pass} --path ./src/resources/wallet-test.json`, () => {
            const readWalletMnemonic = fs.readFileSync('../src/resources/wallet-test.json', 'utf8');
            expect(JSON.parse(readWalletMnemonic).ethWallet.address).to.be.equal('ea7863f14d1a38db7a5e937178fdb7dfa9c96ed7');
            done();
        });
    });
    ...
    it('createkeys import error', (done) => {
        const out = process.exec(`cd ..; node cli.js createkeys --keytype rollup --import ./src/resources/wallet-test.json --pass ${pass} --path ./src/resources/walletRollupImport.json`);
        out.on('exit', (code) => {
            expect(code).to.not.be.equal(0);
            done();
        });
