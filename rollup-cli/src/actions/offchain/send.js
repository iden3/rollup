...
function send(UrlOperator, idTo, amount, BabyjubJson, password) {
  ...
  return new Promise((resolve, reject) => {
    axios.get(`${UrlOperator}/offchain/info/${walletBaby.publicKey.toString()}`).then((response) => {
      const transaction = {
      ...
      axios.post(`${UrlOperator}/offchain/send`, { transaction, sign }).then((responsePost) => {
        resolve(responsePost.status);
      })
        .catch((error) => {
          reject(error);
        });
