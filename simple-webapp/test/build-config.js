...
const synchConfigPath = path.join(__dirname, "../../rollup-operator/test/config/synch-config-pob-test.json");
const testPath = path.join(__dirname, "../../rollup-operator/test/config/test-pob.json");
...
function readFiles() {
    ...
    const testAddress = JSON.parse(fs.readFileSync(synchConfigPath, "utf-8"));
    const config = {
        ...
        abiTokens: abiTokens,
        tokensAddress: testAddress.tokenAddress,
    };
    return config;
}
